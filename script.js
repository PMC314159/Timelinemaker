...
const inputs = [
  'pairName', 'pairRoman', 'pairSubtitle', 'themeSong', 'themeArtist', 'layoutPreset', 'pairColor', 'bgColor',
  'charAColor', 'charBColor', 'charAName', 'charASign', 'charAKeywords', 'charABio',
  'charBName', 'charBSign', 'charBKeywords', 'charBBio', 'charAStory', 'charBStory'
];
...
function buildForms() {
  ...
  wrap.innerHTML = '';
  wrap.className = `timeline-flow count-${state.blocks.length}`;
...
function render() {
  ...
  const preset = $('layoutPreset').value || 'magazine';
  ...
  $('captureArea').className = `timeline-sheet preset-${preset}`;

  $('previewPairName').textContent = $('pairName').value || '페어명';
  $('previewPairRoman').textContent = $('pairRoman').value || 'TIMELINE';
  $('previewSubtitle').textContent = $('pairSubtitle').value || '부제';
  $('previewASign').textContent = $('charASign').value || '';
  $('previewBSign').textContent = $('charBSign').value || '';
  renderTheme();
...
function renderTheme() {
  const wrap = $('themePreview');
  const song = $('themeSong').value.trim();
  const artist = $('themeArtist').value.trim();
  wrap.innerHTML = '';
  wrap.style.display = song || artist ? 'block' : 'none';
  if (!song && !artist) return;
  wrap.innerHTML = `${song ? `<strong>♪ ${escapeText(song)}</strong>` : ''}${artist ? `<span>${escapeText(artist)}</span>` : ''}`;
}
...
function renderTimeline() {
  ...
  state.blocks.forEach((block, index) => {
    ...
    const quotes = [];
    if (block.quoteA.trim()) quotes.push(`<div class="quote-box quote-a"><strong>${escapeText($('charAName').value || 'A')}</strong>${escapeText(block.quoteA)}</div>`);
    if (block.quoteB.trim()) quotes.push(`<div class="quote-box quote-b"><strong>${escapeText($('charBName').value || 'B')}</strong>${escapeText(block.quoteB)}</div>`);
    if (quotes.length) parts.push(`<div class="quotes">${quotes.join('')}</div>`);
